...
LoginButtonStatus button_status = LOGIN;
...
void MainWindow::init_signal() {
  auto login_slot = [&] { on_login_button_clicked(); };
  ...
  auto_button_->signal_toggled().connect([&]() {
    if (auto_button_->get_active()) {
      remember_button_->set_active(true);
    }
  });
  remember_button_->signal_toggled().connect([&]() {
  ...
  drcom_.signal_login().connect(sigc::mem_fun(this, &MainWindow::on_login_done));
  drcom_.signal_logout().connect(sigc::mem_fun(this, &MainWindow::on_logout_done));
  drcom_.signal_abort().connect(sigc::mem_fun(this, &MainWindow::on_alive_abort));
}
...
void MainWindow::on_login_done(bool success, const std::string &msg) {
  if (!success) {
    ...
    button_status = LOGIN;
    Gtk::MessageDialog dialog(*this, msg, false, Gtk::MESSAGE_WARNING, Gtk::BUTTONS_OK);
    dialog.run();
    return;
  }
  login_button_->set_label("Logout");
  button_status = LOGOUT;
